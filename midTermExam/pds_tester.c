...
void process_line( char *test_case )
{
	...
	if( !strcmp(command,"OPEN") ){
		...
		sscanf(param1, "%d", &contact_id);
		testContact.contact_id = contact_id;
		sprintf(testContact.contact_name,"Updated-Name-of-%d",contact_id);
		sprintf(testContact.phone,"Updated-Phone-of-%d",contact_id);
		status = overwrite_contact( &testContact );
		if( status != expected_status ){
			sprintf(info,"Update contact_id: %d; Got status %d",contact_id, status);
			TREPORT("FAIL", info);
		}
			...
			if( status != CONTACT_SUCCESS ){
				...
				// Check if the retrieved values match
				sprintf(expected_name,"Updated-Name-of-%d",contact_id);
				sprintf(expected_phone,"Updated-Phone-of-%d",contact_id);
				if (testContact.contact_id == contact_id && 
					strcmp(testContact.contact_name,expected_name) == 0 &&
					strcmp(testContact.phone,expected_phone) == 0){
						TREPORT("PASS", "");
					...
					sprintf(info,"Updated Contact data not matching. Expected:{%d,%s,%s} Got:{%d,%s,%s}\n",
							contact_id, expected_name, expected_phone, 
							testContact.contact_id, testContact.contact_name, testContact.phone
						);
					TREPORT("FAIL", info);
